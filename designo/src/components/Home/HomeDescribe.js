       ...
       <div className='w-4/5 md:w-11/12'>
            {
                describeData.map(index => 
                    <div key={index.key + index.name} className="flex flex-col items-center text-center text-myDarkGrey md:flex-row md:text-left md:justify-between">
                        <img src={index.svg} alt={index.name} className="w-52"/>
                        <div className='md:w-[439px] md:flex md:flex-col md:items-start'>
                            <h3 className='mt-12 font-medium text-xl tracking-[5px] md:mt-0 md:leading-6'>{index.name}</h3>
                            <p className='mt-8 text-xs font-normal leading-7 md:mt-4'>{index.text}</p>
                        </div>
                    </div>
                )
            }
        ...
        <div className='w-4/5 h-[379px] bg-peach mt-28 rounded-2xl text-myWhite flex flex-col items-center justify-center text-center z-10 overflow-hidden relative md:w-11/12'>
        <svg width="876" height="584" xmlns="http://www.w3.org/2000/svg" className='absolute ml-20'><defs><linearGradient x1="0%" y1="50%" x2="100%" y2="50%" id="a"><stop stopColor="#5D0202" stopOpacity="0" offset="0%"/><stop stopColor="#5D0202" stopOpacity=".498" offset="100%"/></linearGradient></defs><g transform="matrix(-1 0 0 1 876 0)" fill="url(#a)" fillRule="evenodd" opacity=".309"><g transform="translate(0 292)"><circle transform="matrix(0 -1 -1 0 292 292)" cx="146" cy="146" r="146"/><circle transform="matrix(-1 0 0 1 876 0)" cx="438" cy="146" r="146"/><circle transform="matrix(0 1 1 0 584 -584)" cx="730" cy="146" r="146"/></g><circle transform="matrix(0 -1 -1 0 292 292)" cx="146" cy="146" r="146"/><circle transform="matrix(-1 0 0 1 876 0)" cx="438" cy="146" r="146"/></g></svg>
            <h2 className='font-medium text-[32px] leading-[36px] w-2/3 md:text-[40px] md:w-72'>Let’s talk about your project</h2>
            <p className='text-sm leading-6 w-5/6 mt-2 md:text-base md:w-[573px]'>Ready to take it to the next level? Contact us today and find out how our expertise can help your business grow.</p>
